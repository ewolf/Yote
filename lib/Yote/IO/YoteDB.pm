...
weak references, then it can be removed";
    ...
    for my $wf (@weaks) {
      my( $id, $obj ) = @$wf;
      if ( ref( $obj ) eq 'ARRAY' ) {
        for ( grep { $weak_only_check{$_} }
              map { Yote::ObjProvider::xform_in($_) } @$obj ) {
          $weak_only_check{ $_ }++;
        }
        ...
        for ( grep { $weak_only_check{$_} } map { Yote::ObjProvider::xform_in($_) } values %{$obj->{DATA}} ) {
          $weak_only_check{ $_ }++;
        }
      }
    } #each weak
