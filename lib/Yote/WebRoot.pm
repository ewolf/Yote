...
#
# Returns singleton Yote::WebRoot object.
#
sub fetch_webroot {
    $Yote::WebRoot::ROOT_INIT = 1;
    my $root = Yote::Root::fetch_root();
    my $webroot = $root->get_webroot;
    unless( $webroot ) {
        $webroot = new Yote::WebRoot();
        $root->set_webroot( $webroot );
        Yote::ObjProvider::stow( $root );
        Yote::ObjProvider::stow( $webroot );
    }
    $Yote::WebRoot::ROOT_INIT = 0;
    return $webroot;
} #fetch_webroot
