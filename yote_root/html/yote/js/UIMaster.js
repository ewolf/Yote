UIMaster = {
    ...
    make_control:function() {
        ...
        var control = {
            ...
            accept_sprite_from_control:function( sprite, control ) {
                this.selector().append( this.master.sprite_selectors[ sprite.id ] );
                this.sprites.push( sprite );
                this.update_sprites();
            },
            ...
            give_sprite_to_control:function( sprite, control ) {
                if( control.can_accept_sprite( sprite, this ) ) {
                    ...
                    control.accept_sprite_from_control( sprite, this );
                }
                this.update_sprites();
            },
	...
	for( var idx in list ) {
            ctrl.accept_sprite( make_title_sprite( list[idx] ) );
	}
...
$().ready(function() {
    ...
    if( false ) {
        ...
        var bluebox = make_list_box( [ "A", "C" ] );
        bluebox.selector().css('background-color','lightblue').height( 500 ).width( 105 );

        console.log( bluebox );

        var greenbox = make_list_box( [ "B" ] );
        greenbox.selector().css('background-color','lightgreen').height( 500 ).width( 105 ).offset( { left:120, top:bluebox.selector().offset().top } );
        return;
    }
