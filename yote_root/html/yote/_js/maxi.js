...
*/
	...
	remove_from_list : function( args ) {
		...
		var container = args.default_parent.wrap_list( { collection_name : args.field, wrap_key : args.parent_template_id }, true );
    ...
    register_template_value:function( text_val, params ) { //expects "_name_ (new(_hashkey)?)? <control>"
	  ...
	  <$$$ function foo() { ... } $$$>
         */
	var parts = text_val.match( /^\s*(var|control|function|new|new_hashkey)\s+((\S+)([\s\S]*))?/i );
	if( parts ) {
            var cmd = parts[ 1 ];
            var varname = parts[ 3 ];
		...
		var args = $.yote.util.clone_template_args( params );
		var listparts = parts[ 4 ].split(/ +/);
		if( listparts.length == 2 ) {
		    args.template = listparts[ 0 ];
