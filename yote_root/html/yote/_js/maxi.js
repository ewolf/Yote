...
(function() { 
    ...
    var _data_wrapper = function( struct, key, is_hash ) {
        var arry = struct;
        var hash = struct;
    ...
    var _compile_template = function( key, value ) {
        key = key.trim();
	// fun list = a list of ( priority, function ) couples
	var fun_list = _parse_template( value, key );
	...
	for( var i=0, len=idxs.length; i<len; i++ ) {
		...
		if( priority == 8 ) { //after render priority
			...
			var ret = _fill_template_direct( item( ctx ), ctx, key );
			return ret;
		    }, false, false ] );
    ...
    var _parse_args = function( arg_txt ) {
		...
		if( parts[ 4 ].trim().length > 0 ) {
		    var newparts = _parse_args( parts[ 4 ] );
    ...
    var _parse_template = function( template_txt, template_name, recurse ) {
        // clear out comments
        //if( $.yote.templates.debug ) return;
        if( ! recurse ) {
	    while( template_txt.indexOf( '<#' ) > -1 ) {
	        var parts = _template_parts( template_txt, '#', template_name );
	...
	if( recurse < 2 && template_txt.indexOf( '<???' ) > -1 ) {
	    var parts = _template_parts( template_txt, '???', template_name );
            var A = _parse_template( parts[ 0 ], template_name, 2 );
            var B = _parse_template( parts[ 2 ], template_name, 1 );
	    try {
                var f = _to_function( parts[1] );
                A.push( [ 0, f ] );
                A.push.apply( A, B );
                return A;
	    }
	...
	if( recurse < 3 && template_txt.indexOf( '<$$$' ) > -1 ) {
	    var parts = _template_parts( template_txt, '$$$', template_name );
            var A = _parse_template( parts[ 0 ], template_name, 3 );
            var B = _parse_template( parts[ 2 ], template_name, 2 );
            A.push( [ 1, function( ctx ) { return _register( parts[ 1 ], ctx )  } ] );
            A.push.apply( A, B );
            return A;
	} // $$$
	    ...
	    var parts = _template_parts( template_txt, '??', template_name );
            var A = _parse_template( parts[ 0 ], template_name, 4 );
            var B = _parse_template( parts[ 2 ], template_name, 3 );
	    try {
		var f = _to_function( parts[1] );
                A.push( [ 3, f ] );
                A.push.apply( A, B );
	...
	if( recurse < 7 && template_txt.indexOf( '<$$' ) > -1 ) {
	    var parts = _template_parts( template_txt, '$$', template_name );
	    var args = _parse_args( parts[1] );
            var tmpl = args.shift();
            var A = _parse_template( parts[ 0 ], template_name, 7 );
            var B = _parse_template( parts[ 2 ], template_name, 6 );
            A.push( [ 5, function( ctx ) { return _fill_template( tmpl, ctx, args )  } ] );
            A.push.apply( A, B );
            return A;
	} // $$
	...
	if( recurse < 8 && template_txt.indexOf( '<$' ) > -1 ) {
	    ...
	    var args = _parse_args( parts[1] );
            var vari = args.shift();
            var A = _parse_template( parts[ 0 ], template_name, 8 );
            var B = _parse_template( parts[ 2 ], template_name, 7 );
            A.push( [ 6, function( ctx ) { return _fill_template_variable( vari, ctx, args )  } ] );
            A.push.apply( A, B );
            return A;
	}
	...
	if( recurse < 9 && template_txt.indexOf( '<?' ) > -1 ) {
	    var parts = _template_parts( template_txt, '?', template_name );
            var A = _parse_template( parts[ 0 ], template_name, 8 );
            A.push.apply( A, _parse_template( parts[ 2 ], template_name, 7 ) );
	    try {
		var fun = _to_function( parts[1] );
    ...
    // fill
    var _template_parts = function( txt, sigil, template_name ) {
    ...
    var _parse_val = function( value, context, no_literal ) {
        /*
        ...
        if( prev_is_get && last_sep > 0 ) {
            if( is_list ) {
                ...
                subj = subj.class == 'ARRAY' ?
                    subj = _wrap_list( subj.get( vari ).to_list(), context.template_path + '#' + orig_val ) :
                    subj = $.yote._wrap_list( subj, vari, context.template_path + '#' + orig_val );
            } else if( is_wrapped_hash ) {
                subj = subj.class == 'HASH' ?
                    _wrap_hash( subj.get( vari ).to_hash(), context.template_path + '#' + orig_val ) :
            ...
            if( is_wrapped_list ) {
                subj = _wrap_list( subj, context.template_path + '#' + orig_val );
            } else if( is_wrapped_hash ) {
                subj = _wrap_hash( subj, context.template_path + '#' + orig_val );
            } 
        }
    ...
    var _register = function( args_string, context ) {
        /*
    ...
    var _fill_template = function( template_name, old_context, args ) {
	var compilation = _compiled_templates[ template_name ];
	if( ! compilation ) {
	    console.log( "Error : Template '" + template_name + '" not found.' );
	    return '';
	}
	var context = old_context ? old_context.clone() : _new_context();
    ...
    var _fill_template_direct = function( template, context, template_name ) {
        if( ! template ) return '';
	template += '';

	while( template.indexOf( '<#' ) > -1 ) {
	    var parts = _template_parts( template, '#', template_name );
	...
	while( template.indexOf( '<$$' ) > -1 ) {
	    ...
	    template = parts[ 0 ] +
		_fill_template( template_name, context, args ) +
		parts[ 2 ];
	} // $$
	...
	while( template.indexOf( '<$' ) > -1 ) {
	    ...
	    var vari = args.shift();
	    template = parts[ 0 ] +
		_fill_template_variable( vari, context, args ) +
		parts[ 2 ];
	}
	while( template.indexOf( '<?' ) > -1 ) {
	    // functions to be run after rendering is done
    ...
    var _fill_template_variable = function( vari, context, args ) {
	var res = _parse_val( vari, context, true );
	return typeof res === 'undefined' ? args[ 0 ] ? args[ 0 ] : '' : res;
    }; //_fill_template_variable

    
    $.yote.templates = {

        // called seldom/debugging
        fill_template:function( template_name, old_context, args ) {
            return _fill_template( template_name, old_context, args );
        },
		...
		return;
            ...
            var initial = $el.html();
	    try {
                var filled = _fill_template( templ_name, undefined, [] );
